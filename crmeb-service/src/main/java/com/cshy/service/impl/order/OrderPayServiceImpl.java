    ...
    public Boolean paySuccess(StoreOrder storeOrder) {
        ...
        if (execute) {
            try {
                ...
                shortenURL = shortenURL.substring(shortenURL.lastIndexOf("/"));

                //短信通知员工
//                smsService.sendSMS(null, SmsTriggerEnum.ORDER_PLACED_TO_EMPLOYEE.getCode(), null, "普通");
                //短信通知客户
//                smsService.sendSMS(storeOrder.getUserMobile(), SmsTriggerEnum.ORDER_PLACED_TO_CUSTOMER.getCode(), null, "购买", shortenURL.substring(shortenURL.lastIndexOf("/")));
                //构建公众号消息模板参数
                List<String> list = Lists.newArrayList();
                list.add(String.valueOf(storeOrder.getPayPrice()));
                list.add("测试");
                list.add(storeOrder.getOrderId());
                list.add(DateUtil.nowDateTime(DateConstants.DATE_FORMAT));
                list.add("12345");
                systemNotificationService.sendNotification(NotifyTypeEnum.PAY_SUCCESS.getCode(), storeOrder.getUid(), list, "购买", shortenURL);
                // 购买成功后根据配置送优惠券
...
//            }
    ...
    private Map<String, String> unifiedorder(StoreOrder storeOrder, String ip) {
        ...
        // 预下单（统一下单）
        CreateOrderResponseVo responseVo = wechatCommonService.payUnifiedorder(unifiedorderVo);
        // 组装前端预下单参数
    ...
    private void pushMessageOrder(StoreOrder storeOrder, User user, SystemNotification payNotification) {
        if (storeOrder.getPaymentChannel().equals(2)) {// H5
            return;
        }
        UserToken userToken;
        HashMap<String, String> temMap = new HashMap<>();
        ...
        // 公众号
        if (storeOrder.getPaymentChannel().equals(Constants.ORDER_PAY_CHANNEL_PUBLIC) && payNotification.getIsWechat().equals(1)) {
            userToken = userTokenService.getTokenByUserId(user.getUid(), UserConstants.USER_TOKEN_TYPE_WECHAT);
            if (ObjectUtil.isNull(userToken)) {
                return ;
            }
            ...
            sysTemplateMessageService.pushTemplateMessage(payNotification.getWechatId(), temMap, userToken.getToken());
            return;
        }
        if (storeOrder.getPaymentChannel().equals(Constants.ORDER_PAY_CHANNEL_PROGRAM) && payNotification.getIsRoutine().equals(1)) {
            // 小程序发送订阅消息
            userToken = userTokenService.getTokenByUserId(user.getUid(), UserConstants.USER_TOKEN_TYPE_ROUTINE);
            if (ObjectUtil.isNull(userToken)) {
                return ;
...
//            temMap.put("thing7", "您的订单已支付成功");
            ...
            sysTemplateMessageService.pushMiniTemplateMessage(payNotification.getRoutineId(), temMap, userToken.getToken());
        }
    }
