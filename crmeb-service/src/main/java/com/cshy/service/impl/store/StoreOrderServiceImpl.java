...
public class StoreOrderServiceImpl extends ServiceImpl<StoreOrderDao, StoreOrder> implements StoreOrderService {
    ...
    @Autowired
    private com.cshy.service.service.store.StoreOrderInfoService StoreOrderInfoService;
    ...
    public CommonPage<StoreOrderDetailResponse> getAdminList(StoreOrderSearchRequest request, PageParamRequest pageParamRequest) {
        ...
        queryWrapper.select("id", "order_id", "uid", "real_name", "pay_price", "pay_type", "create_time", "status", "refund_status"
                ...
                , "is_del", "combination_id", "pink_id", "seckill_id", "bargain_id", "verify_code", "remark", "paid", "is_system_del", "shipping_type", "type", "is_alter_price", "refund_type");
     ...
     */
    @Override
    public List<StoreOrder> getUserOrderList(Integer uid, Map<String, Object> params, PageParamRequest pageParamRequest) {
        Integer status = Objects.nonNull(params.get("type")) ? Integer.valueOf((String) params.get("type")) : null;
        ...
        //查询商品名称
        lqw.and(StringUtils.isNotBlank(condition), wrapper -> {
            wrapper.apply("s_order.id in (select order_id from s_order_info where product_name like '%" + condition + "%')");
            wrapper.or().like(StoreOrder::getOrderId, condition);
        });
...
//            }
    ...
    public Boolean refundRefuse(String orderNo, String reason) {
        ...
        Boolean execute = transactionTemplate.execute(e -> {
            updateById(storeOrder);
            storeOrderStatusService.createLog(storeOrder.getId(), StoreOrderStatusConstants.ORDER_LOG_REFUND_REFUSE, MsgConstants.ORDER_LOG_MESSAGE_REFUND_REFUSE.replace("{reason}", reason), 1);
            return Boolean.TRUE;
        });
        //TODO
//        if (execute) {
...
//        }
    ...
    public Boolean updatePrice(StoreOrderUpdatePriceRequest request) {
        ...
        Boolean execute = transactionTemplate.execute(e -> {
            ...
            // 订单修改状态操作
            storeOrderStatusService.createLog(existOrder.getId(), StoreOrderStatusConstants.ORDER_LOG_EDIT,
                    MsgConstants.RESULT_ORDER_EDIT_PRICE_LOGS.replace("${orderPrice}", oldPrice)
                            .replace("${price}", request.getPayPrice() + ""), 1);
            return Boolean.TRUE;
        });
        if (!execute) {
            throw new CrmebException(MsgConstants.RESULT_ORDER_EDIT_PRICE_SUCCESS
    ...
    public StoreOrderCountItemResponse getOrderStatusNum(String dateLimit, Integer type) {
        ...
        // 全部订单
        response.setAll(getCount(dateLimit, StoreOrderStatusConstants.ORDER_STATUS_ALL, type));
        // 未支付订单
        response.setUnPaid(getCount(dateLimit, StoreOrderStatusConstants.ORDER_STATUS_UNPAID, type));
        // 未发货订单
        response.setNotShipped(getCount(dateLimit, StoreOrderStatusConstants.ORDER_STATUS_NOT_SHIPPED, type));
        // 待收货订单
        response.setSpike(getCount(dateLimit, StoreOrderStatusConstants.ORDER_STATUS_SPIKE, type));
        // 待评价订单
        response.setBargain(getCount(dateLimit, StoreOrderStatusConstants.ORDER_STATUS_BARGAIN, type));
        // 交易完成订单
        response.setComplete(getCount(dateLimit, StoreOrderStatusConstants.ORDER_STATUS_COMPLETE, type));
        // 待核销订单
        response.setToBeWrittenOff(getCount(dateLimit, StoreOrderStatusConstants.ORDER_STATUS_TOBE_WRITTEN_OFF, type));
        // 退款中订单
        response.setRefunding(getCount(dateLimit, StoreOrderStatusConstants.ORDER_STATUS_REFUNDING, type));
        // 已退款订单
    ...
    public StoreOrderTopItemResponse getOrderData(String dateLimit) {
        ...
        // 订单数量
        itemResponse.setCount(getCount(dateLimit, StoreOrderStatusConstants.ORDER_STATUS_ALL));
        // 订单金额
...
//        wrapper.select("ifnull(if(sum(pay_price) = 0.00, 0, sum(pay_price)), 0) as pay_price");
    ...
    public Integer getNotShippingNum() {
        return getCount("", StoreOrderStatusConstants.ORDER_STATUS_NOT_SHIPPED);
    }
    ...
    @Override
    public Boolean refundNReturn(StoreOrderRefundRequest request) {
        Assert.notNull(request.getReturnName(), "退货收件人不能为空");
        Assert.notNull(request.getReturnAddress(), "退货收件地址不能为空");
        Assert.notNull(request.getReturnMobile(), "退货收件电话不能为空");
        ...
        storeOrder.setRefundPrice(request.getAmount());
        storeOrder.setReturnAddress(request.getReturnAddress());
        storeOrder.setReturnMobile(request.getReturnMobile());
        storeOrder.setReturnName(request.getReturnName());
        Boolean execute = transactionTemplate.execute(e -> {
            updateById(storeOrder);
            return Boolean.TRUE;
        });
        if (!execute) {
            ...
            storeOrderStatusService.createLog(storeOrder.getId(), StoreOrderStatusConstants.ORDER_LOG_AGREE_RETURN, StoreOrderStatusConstants.ORDER_STATUS_STR_AGREE_RETURN, 1);
        }
        return execute;
    }
    ...
    public Boolean returnShip(String orderId, String trackingNo, String remark, String img) {
        ...
        storeOrder.setRefundTrackingNo(trackingNo);
        storeOrder.setReturnRemark(remark);
        storeOrder.setReturnImg(systemAttachmentService.clearPrefix(img));
        storeOrder.setRefundStatus(5);
        //创建日志
        storeOrderStatusService.createLog(storeOrder.getId(), StoreOrderStatusConstants.ORDER_LOG_RETURN_GOODS, "物流单号：" + trackingNo, 0);
        boolean b = this.updateById(storeOrder);
        if (!b)
    ...
    @Override
    public Boolean returnArrived(String orderId) {
        StoreOrder storeOrder = this.getByOrderIdException(orderId);

        StoreOrderRefundRequest storeOrderRefundRequest = new StoreOrderRefundRequest();
        storeOrderRefundRequest.setOrderId(storeOrder.getId());
        storeOrderRefundRequest.setAmount(storeOrder.getRefundPrice());
        storeOrderRefundRequest.setOrderNo(orderId);
        boolean refund = this.refund(storeOrderRefundRequest);
        return refund;
    }
...
///////////////////////////////////////////////////////////////////////////////////////////////////// 以下为自定义方法
    ...
    private StoreOrder getByOrderIdException(String orderId) {
        StoreOrder storeOrder = getByOrderId(orderId);
    ...
    private void express(StoreOrderSendRequest request, StoreOrder storeOrder) {
        ...
        String message = MsgConstants.ORDER_LOG_MESSAGE_EXPRESS.replace("{deliveryName}", express.getName()).replace("{deliveryCode}", storeOrder.getTrackingNo());

        Boolean execute = transactionTemplate.execute(i -> {
            updateById(storeOrder);
            //订单记录增加
            storeOrderStatusService.createLog(request.getId(), StoreOrderStatusConstants.ORDER_LOG_EXPRESS, message, 1);
            return Boolean.TRUE;
        });
    ...
    private void pushMessageOrder(StoreOrder storeOrder, User user, SystemNotification notification) {
        ...
        // 公众号
        if (storeOrder.getPaymentChannel().equals(Constants.ORDER_PAY_CHANNEL_PUBLIC) && notification.getIsWechat().equals(1)) {
            ...
            sysTemplateMessageService.pushTemplateMessage(notification.getWechatId(), temMap, userToken.getToken());
            return;
...
//        temMap.put("thing7", "您的订单已发货");
            ...
            sysTemplateMessageService.pushMiniTemplateMessage(notification.getRoutineId(), temMap, userToken.getToken());
        }
    }
    ...
    private void expressDump(StoreOrderSendRequest request, StoreOrder storeOrder, Express express) {
        ...
        record.set("to_tel", storeOrder.getUserMobile());// 收件人电话
        record.set("to_addr", storeOrder.getAddress() + storeOrder.getAddressDetail());// 收件人详细地址
        ...
        storeOrder.setTrackingNo(myRecord.getStr("kuaidinum"));
    }
    ...
    private void deliverGoods(StoreOrderSendRequest request, StoreOrder storeOrder) {
        storeOrder.setTrackingNo(request.getExpressNumber());
    }
    ...
    private void delivery(StoreOrderSendRequest request, StoreOrder storeOrder) {
        ...
        storeOrder.setTrackingNo(request.getDeliveryTel());
        storeOrder.setStatus(1);
        ...
        String message = MsgConstants.ORDER_LOG_MESSAGE_DELIVERY.replace("{deliveryName}", request.getDeliveryName()).replace("{deliveryCode}", request.getDeliveryTel());
...
//            map.put("keyword3", storeOrder.getUserAddress());
            ...
            // 发送微信模板消息
            sysTemplateMessageService.pushTemplateMessage(notification.getWechatId(), map, userToken.getToken());
        } else if (notification.getIsRoutine().equals(1)) {
            // 小程序发送订阅消息
            userToken = userTokenService.getTokenByUserId(user.getUid(), UserConstants.USER_TOKEN_TYPE_ROUTINE);
            if (ObjectUtil.isNull(userToken)) {
...
//        map.put("phone_number10", request.getDeliveryTel());
            ...
            map.put("name4", request.getDeliveryName());
            map.put("phone_number10", request.getDeliveryTel());
            sysTemplateMessageService.pushMiniTemplateMessage(notification.getRoutineId(), map, userToken.getToken());
        }
    }
    ...
    private void virtual(StoreOrderSendRequest request, StoreOrder storeOrder) {
        ...
        Boolean execute = transactionTemplate.execute(i -> {
            updateById(storeOrder);
            //订单记录增加
            storeOrderStatusService.createLog(request.getId(), StoreOrderStatusConstants.ORDER_LOG_DELIVERY_VI, "虚拟物品发货", 1);
            return Boolean.TRUE;
        });
    ...
    private void getStatusWhereNew(QueryWrapper<StoreOrder> queryWrapper, String status) {
        ...
        switch (status) {
            case StoreOrderStatusConstants.ORDER_STATUS_ALL: //全部
                break;
            case StoreOrderStatusConstants.ORDER_STATUS_UNPAID: //未支付
                queryWrapper.eq("paid", 0);//支付状态
                queryWrapper.eq("status", 0); //订单状态
                queryWrapper.eq("is_del", 0);//删除状态
                break;
            case StoreOrderStatusConstants.ORDER_STATUS_NOT_SHIPPED: //未发货
                queryWrapper.eq("paid", 1);
                ...
                break;
            case StoreOrderStatusConstants.ORDER_STATUS_SPIKE: //待收货
                queryWrapper.eq("paid", 1);
                queryWrapper.eq("status", 1);
                queryWrapper.eq("refund_status", 0);
                queryWrapper.eq("is_del", 0);
                break;
            case StoreOrderStatusConstants.ORDER_STATUS_BARGAIN: //待评价
                queryWrapper.eq("paid", 1);
                queryWrapper.eq("status", 2);
                queryWrapper.eq("refund_status", 0);
                queryWrapper.eq("is_del", 0);
                break;
            case StoreOrderStatusConstants.ORDER_STATUS_COMPLETE: //交易完成
                queryWrapper.eq("paid", 1);
                queryWrapper.eq("status", 3);
                queryWrapper.eq("refund_status", 0);
                queryWrapper.eq("is_del", 0);
                break;
            case StoreOrderStatusConstants.ORDER_STATUS_TOBE_WRITTEN_OFF: //待核销
                queryWrapper.eq("paid", 1);
                queryWrapper.eq("status", 0);
                queryWrapper.eq("refund_status", 0);
                ...
                queryWrapper.eq("is_del", 0);
                break;
            case StoreOrderStatusConstants.ORDER_STATUS_REFUNDING: //退款中
                queryWrapper.eq("paid", 1);
                queryWrapper.in("refund_status", 1, 3);
                queryWrapper.eq("is_del", 0);
                break;
            case StoreOrderStatusConstants.ORDER_STATUS_REFUNDED: //已退款
                queryWrapper.eq("paid", 1);
    ...
    private Map<String, String> getStatus(StoreOrder storeOrder) {
        ...
        // 未发货
        if (storeOrder.getPaid()
                && ((storeOrder.getStatus() == 0 || storeOrder.getRefundStatus() == 6 || storeOrder.getRefundStatus() == 7)
                ...
                && !storeOrder.getIsDel()
                && !storeOrder.getIsSystemDel()) {
            map.put("key", StoreOrderStatusConstants.ORDER_STATUS_NOT_SHIPPED);
            map.put("value", StoreOrderStatusConstants.ORDER_STATUS_STR_NOT_SHIPPING);
            return map;
        }
        // 待收货
        if (storeOrder.getPaid()
                && ((storeOrder.getStatus() == 1 || storeOrder.getRefundStatus() == 6 || storeOrder.getRefundStatus() == 7)
                && storeOrder.getRefundStatus() != 1
                && storeOrder.getRefundStatus() != 2
                && storeOrder.getRefundStatus() != 3
                && storeOrder.getRefundStatus() != 4
                ...
                && !storeOrder.getIsDel()
                && !storeOrder.getIsSystemDel()) {
            map.put("key", StoreOrderStatusConstants.ORDER_STATUS_SPIKE);
            map.put("value", StoreOrderStatusConstants.ORDER_STATUS_STR_SPIKE);
            return map;
        }
        // 待评价
        if (storeOrder.getPaid()
                && ((storeOrder.getStatus() == 2 || storeOrder.getRefundStatus() == 6 || storeOrder.getRefundStatus() == 7)
                && storeOrder.getRefundStatus() != 1
                && storeOrder.getRefundStatus() != 2
                && storeOrder.getRefundStatus() != 3
                && storeOrder.getRefundStatus() != 4
                ...
                && !storeOrder.getIsDel()
                && !storeOrder.getIsSystemDel()) {
            map.put("key", StoreOrderStatusConstants.ORDER_STATUS_BARGAIN);
            map.put("value", StoreOrderStatusConstants.ORDER_STATUS_STR_BARGAIN);
            return map;
        }
        // 交易完成
        if (storeOrder.getPaid()
                && ((storeOrder.getStatus() == 3 || storeOrder.getRefundStatus() == 6 || storeOrder.getRefundStatus() == 7)
                && storeOrder.getRefundStatus() != 1
                && storeOrder.getRefundStatus() != 2
                && storeOrder.getRefundStatus() != 3
                && storeOrder.getRefundStatus() != 4
                ...
                && !storeOrder.getIsDel()
                && !storeOrder.getIsSystemDel()) {
            map.put("key", StoreOrderStatusConstants.ORDER_STATUS_COMPLETE);
            map.put("value", StoreOrderStatusConstants.ORDER_STATUS_STR_COMPLETE);
            return map;
        }
        // 待核销
        if (storeOrder.getPaid()
                ...
                && !storeOrder.getIsDel()
                && !storeOrder.getIsSystemDel()) {
            map.put("key", StoreOrderStatusConstants.ORDER_STATUS_TOBE_WRITTEN_OFF);
            map.put("value", StoreOrderStatusConstants.ORDER_STATUS_STR_TOBE_WRITTEN_OFF);
            return map;
        }
        ...
        if (storeOrder.getPaid()
                ...
                && !storeOrder.getIsDel()
                && !storeOrder.getIsSystemDel()
                && Objects.nonNull(storeOrder.getRefundType())) {
            if (storeOrder.getRefundType().equals(0)) {
                map.put("key", StoreOrderStatusConstants.ORDER_STATUS_APPLY_REFUNDING);
                map.put("value", StoreOrderStatusConstants.ORDER_STATUS_STR_APPLY_REFUNDING);
            } else {
                map.put("key", StoreOrderStatusConstants.ORDER_STATUS_APPLY_REFUNDING_RETURN);
                map.put("value", StoreOrderStatusConstants.ORDER_STATUS_STR_APPLY_REFUNDING_RETURN);
            }
            return map;
        }
        ...
        if (storeOrder.getPaid()
                ...
                && !storeOrder.getIsDel()
                && !storeOrder.getIsSystemDel()) {
            map.put("key", StoreOrderStatusConstants.ORDER_STATUS_REFUNDING);
            map.put("value", StoreOrderStatusConstants.ORDER_STATUS_STR_REFUNDING);
            return map;
        }
        ...
        if (storeOrder.getPaid()
                ...
                && !storeOrder.getIsDel()
                && !storeOrder.getIsSystemDel()) {
            map.put("key", StoreOrderStatusConstants.ORDER_STATUS_REFUNDED);
            map.put("value", StoreOrderStatusConstants.ORDER_STATUS_STR_REFUNDED);
        }

        //已删除
        if (storeOrder.getIsDel() || storeOrder.getIsSystemDel()) {
            map.put("key", StoreOrderStatusConstants.ORDER_STATUS_DELETED);
            map.put("value", StoreOrderStatusConstants.ORDER_STATUS_STR_DELETED);
        }
        ...
        if (storeOrder.getPaid()
                ...
                && !storeOrder.getIsDel()
                && !storeOrder.getIsSystemDel()) {
            map.put("key", StoreOrderStatusConstants.ORDER_STATUS_APPLY_WAIT_FOR_SHIP);
            map.put("value", StoreOrderStatusConstants.ORDER_STATUS_STR_APPLY_WAIT_FOR_SHIP);
        }
        ...
        if (storeOrder.getPaid()
                ...
                && !storeOrder.getIsDel()
                && !storeOrder.getIsSystemDel()) {
            map.put("key", StoreOrderStatusConstants.ORDER_STATUS_APPLY_SHIPPING);
            map.put("value", StoreOrderStatusConstants.ORDER_STATUS_STR_APPLY_SHIPPING);
        }
