...
import com.cshy.service.dao.store.StoreSeckillDao;
...
public class StoreSeckillServiceImpl extends ServiceImpl<StoreSeckillDao, StoreSeckill>
    ...
    public PageInfo<StoreSeckillResponse> getList(StoreSeckillSearchRequest request, PageParamRequest pageParamRequest) {
        ...
        lambdaQueryWrapper.in(StoreSeckill::getTimeId, storeSeckillMangerIdList);
...
//        String currentSkillTime = null;
    ...
    public List<SecKillResponse> getForH5Index() {
        ...
        skillManagerList.forEach(e -> {
            ...
            if (proNum > 0) {
                ...
                SecKillResponse r = new SecKillResponse(e.getId(), e.getSliderImages(), e.getStatusName(),
    ...
    @Override
    public List<Map<String, Object>> queryByCateId(Integer cateId) {
        //判断是二级分类id还是1级分类id
        Category category = categoryService.getById(cateId);
        List<Integer> cateIdList = Lists.newArrayList();
        if (category.getPid().equals(0)) {
            //查询二级分类
            List<Category> childVoListByPid = categoryService.getChildVoListByPid(cateId);
        ...
        List<Integer> secKillManagerIdList = storeSeckillMangerService.list(new LambdaQueryWrapper<StoreSeckillManger>()
                        .eq(StoreSeckillManger::getStatus, 1)
                        .le(StoreSeckillManger::getStartTime, hour)
                        .ge(StoreSeckillManger::getEndTime, hour)
                        .eq(StoreSeckillManger::getIsDel, false)
                        .eq(StoreSeckillManger::getType, 1))
                .stream().map(StoreSeckillManger::getId).collect(Collectors.toList());
        //根据商品id查询秒杀商品
        LambdaQueryWrapper<StoreSeckill> wrapper = new LambdaQueryWrapper<>();
        String currentDate = DateUtil.hutoolDateTime2Str(nowDateTime, DateConstants.DATE_FORMAT_DATE);
        wrapper.in(StoreSeckill::getProductId, productIdList)
                .eq(StoreSeckill::getStatus, 1)
                .le(StoreSeckill::getStartTime, currentDate)
                .ge(StoreSeckill::getStopTime, currentDate)
                .eq(StoreSeckill::getIsDel, false);
        if (CollUtil.isNotEmpty(secKillManagerIdList))
            wrapper.in(StoreSeckill::getTimeId, secKillManagerIdList);
        else
            return Lists.newArrayList();
        List<StoreSeckill> storeSeckillList = list(wrapper);
        //过滤查询出的商品
        List<Integer> secKillProductList = storeSeckillList.stream().map(StoreSeckill::getProductId).collect(Collectors.toList());
        List<Map<String, Object>> resMapList = storeProductList.stream().filter(storeProduct -> secKillProductList.contains(storeProduct.getId())).distinct().map(storeProduct -> {
