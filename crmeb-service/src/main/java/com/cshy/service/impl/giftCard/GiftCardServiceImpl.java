package com.cshy.service.impl.giftCard;
...
public class GiftCardServiceImpl extends BaseServiceImpl<GiftCard, GiftCardDto,
    ...
    @Override
    public List<Map<String, Object>> getByType(String typeId) {
        List<Map<String, Object>> reslist = Lists.newArrayList();
        //查询全部的礼品卡类型
        LambdaQueryWrapper<GiftCardType> lambdaQueryWrapper = new LambdaQueryWrapper<GiftCardType>().eq(GiftCardType::getStatus, 0);
        if (StringUtils.isNotBlank(typeId))
            lambdaQueryWrapper.eq(GiftCardType::getId, typeId);
        List<GiftCardType> giftCardTypeList = giftCardTypeService.list(lambdaQueryWrapper);

        //获取礼品卡类型下所有商品
        giftCardTypeList.forEach(giftCardType -> {
            List<GiftCardProduct> list = giftCardProductService.list(new LambdaQueryWrapper<GiftCardProduct>().eq(GiftCardProduct::getGiftCardTypeId, giftCardType.getId()));
            List<Map<String, Object>> giftCardProductMapList = list.stream().map(giftCardProduct -> {
                //根据productId查询商品信息
                StoreProduct storeProduct = storeProductService.getById(giftCardProduct.getProductId());
                Map<String, Object> map = CommonUtil.objToMap(giftCardProduct, GiftCardProduct.class);
                //放入商品信息
            ...
            Map<String, Object> giftCardTypeMap = CommonUtil.objToMap(giftCardType, GiftCardType.class);
            giftCardTypeMap.put("id", giftCardType.getId());
            giftCardTypeMap.put("productList", giftCardProductMapList);
            reslist.add(giftCardTypeMap);
        });
        return reslist;
    }
