...
public class OrderServiceImpl implements OrderService {
    ...
    public Boolean refundApply(OrderRefundApplyRequest request) {
        ...
        //查询所有订单详情
        List<StoreOrderInfo> storeOrderInfoList = storeOrderInfoService.list(new LambdaQueryWrapper<StoreOrderInfo>().eq(StoreOrderInfo::getOrderId, existStoreOrder.getId()));
        //判断是否是全部退款
        ...
        if (isAllRefund) {
            //全额退款
            refundIntegral = existStoreOrder.getUseIntegral();
            ...
            for (int i = 0; i < request.getRefundOrderInfoRequestList().size(); i++) {
                ...
                if (first.isPresent()) {
                    ...
                    //该种商品单个总价占比
                    BigDecimal rate = storeOrderInfo.getPrice().divide(existStoreOrder.getProTotalPrice(), 10, RoundingMode.HALF_UP);

                    //积分
                    refundIntegral = refundIntegral.add(existStoreOrder.getUseIntegral().subtract(existStoreOrder.getPayPostage()).multiply(rate).multiply(BigDecimal.valueOf(refundOrderInfoRequest.getRefundNum())).setScale(2, RoundingMode.HALF_UP));
                    storeOrderInfo.setRefundNum(refundOrderInfoRequest.getRefundNum());
                }
            }
        }

        if (request.getRefundPrice().compareTo(existStoreOrder.getPayPrice()) > 0)
            throw new CrmebException("退款金额不能大于实际支付金额");
        ...
        existStoreOrder.setRefundPrice(Objects.nonNull(existStoreOrder.getRefundPrice()) ? existStoreOrder.getRefundPrice().add(request.getRefundPrice()) : request.getRefundPrice());
        existStoreOrder.setBackIntegral(Objects.nonNull(existStoreOrder.getBackIntegral()) ? existStoreOrder.getBackIntegral().add(refundIntegral) : refundIntegral);
        ...
        if (execute)
            // 发送用户退款管理员提醒短信
//            smsService.sendSMS(null, SmsTriggerEnum.RETURN_REQUEST_SUBMITTED.getCode(), null, existStoreOrder.getOrderId(), request.getRefundType() == 0 ? "退款" : "退货退款");
...
//        Integer tempId = Integer.valueOf(smsTemplate.getTempId());
    ...
    public CommonPage<OrderDetailResponse> list(Map<String, Object> params, PageParamRequest pageRequest) {
        ...
        for (StoreOrder storeOrder : orderList) {
            ...
            orderInfoList.forEach(orderInfo -> {
                ...
                orderInfoResponse.setAttrId(Integer.valueOf(orderInfo.getUnique()));
                orderInfoResponse.setRefundNum(orderInfo.getRefundNum());
                //查询原价
                StoreProductAttrValue attrValueServiceById = attrValueService.getById(orderInfo.getAttrValueId());
    ...
    private String getH5OrderStatus(StoreOrder storeOrder, Integer type) {
        ...
        if (!storeOrder.getPaid()) {
            ...
            if (storeOrder.getShippingType() == 1)
                ...
                status = "待核销";
        } else if (storeOrder.getStatus().equals(1)) {
            status = "待收货";
        } else if (storeOrder.getStatus().equals(2)) {
            status = "待评价";
        } else if (storeOrder.getStatus().equals(3)) {
            status = "已完成";
        }
        if (!storeOrder.getRefundStatus().equals(0)) {
            if (storeOrder.getRefundStatus().equals(1)) {
                status = "申请退款中";
            } else if (storeOrder.getRefundStatus().equals(2)) {
                status = "已退款";
            } else if (storeOrder.getRefundStatus().equals(4)) {
                status = "退货待发货";
            } else if (storeOrder.getRefundStatus().equals(5)) {
                status = "退货中";
            }
        }
        if (Objects.nonNull(type) && type == -3) {
            if (storeOrder.getRefundStatus().equals(1)) {
                ...
                status = "已退款";
            } else if (storeOrder.getRefundStatus().equals(3)) {
                status = "退款中";
            } else if (storeOrder.getRefundStatus().equals(4)) {
                status = "退货待发货";
            } else if (storeOrder.getRefundStatus().equals(5)) {
                status = "退货中";
            } else if (storeOrder.getRefundStatus().equals(6)) {
                status = "退货退款被拒绝";
            } else if (storeOrder.getRefundStatus().equals(7)) {
                status = "退款被拒绝";
            }
        }
        return status;
    ...
    public StoreOrderDetailInfoResponse detailOrder(String orderId) {
        ...
        infoList.forEach(e -> {
            ...
            orderInfoResponse.setId(e.getId());
            ...
            orderInfoResponse.setRefundNum(e.getRefundNum());
            //查询原价
    ...
    private MyRecord getOrderStatusVo(StoreOrder storeOrder) {
        ...
        try {
            ...
            throw new CrmebException("订单状态异常，请联系管理员");
        }
        return record;
    }
    ...
    public MyRecord preOrder(PreOrderRequest request) {
        ...
        if (orderInfoVo.getOrderDetailList().get(0).getProductType().equals(ProductType.PRODUCT_TYPE_NORMAL)) {
            // 普通商品
...
//        preOrderResponse.setAliPayStatus(aliPayStatus);
    ...
    public MyRecord createOrder(CreateOrderRequest request) {
        ...
        String orderId = handleOrder(orderInfoVo, user, request);

        MyRecord record = new MyRecord();
        record.set("orderNo", Lists.newArrayList(orderId));
        record.set("mainOrderNo", request.getPreOrderNo());
        return record;
    }
...
//        preOrderResponse.setAliPayStatus(aliPayStatus);
    ...
    public void refundTrackingNo(String trackingNo, String uni) {
        ...
        existStoreOrder.setRefundTrackingNo(trackingNo);
        boolean b = storeOrderService.updateById(existStoreOrder);
        if (!b)
            throw new CrmebException("更新订单失败，请稍后重试");
    }
    ...
    public void ship(StoreOrderShipRequest storeOrderShipRequest, HttpServletRequest request) {
        ...
        if (CollUtil.isEmpty(singleOrderRequestList))
            throw new CrmebException("发货数量不能为空");

        boolean anyMatch = singleOrderRequestList.stream().anyMatch(singleOrderRequest -> Objects.isNull(singleOrderRequest.getNum()) || singleOrderRequest.getNum() < 0);
        if (anyMatch)
            throw new CrmebException("发货数量不能为0");
        ...
        if (storeOrderShipRequest.getType() == 0) {
            ...
            singleOrderRequestList.forEach(singleOrderRequest -> {
                //查询订单详情
                Optional<StoreOrderInfo> storeOrderInfoOptional = storeOrderInfoList.stream().filter(storeOrderInfo -> storeOrderInfo.getAttrValueId().equals(singleOrderRequest.getAttrValueId())).findFirst();
...
//        smsService.sendSMS(userMobile, SmsTriggerEnum.ITEMS_SHIPPED.getCode(), request, "/" + param);
    }

    @Override
    public BigDecimal refundCompute(OrderRefundComputeRequest request) {
        //查询订单
        StoreOrder storeOrder = storeOrderService.getById(request.getId());
        //查询订单详情
        ...
        if (isAllRefund) {
            //全额退款
            refundPrice = storeOrder.getPayPrice();
        } else {
            //部分退款
            for (int i = 0; i < request.getRefundOrderInfoRequestList().size(); i++) {
                RefundOrderInfoRequest refundOrderInfoRequest = request.getRefundOrderInfoRequestList().get(i);
                ...
                if (first.isPresent()) {
                    ...
                    //单个应退金额
                    BigDecimal returnPrice = storeOrder.getPayPrice().multiply(rate).setScale(2, RoundingMode.HALF_UP);
                    storeOrderInfo.setRefundNum(refundOrderInfoRequest.getRefundNum());
                    refundPrice = refundPrice.add(returnPrice.multiply(new BigDecimal(refundOrderInfoRequest.getRefundNum())).setScale(2, RoundingMode.HALF_UP));
                }
            }
        }
        return refundPrice;
    ...
    private String handleOrder(OrderInfoVo infoVo, User user, CreateOrderRequest request) {
        // 检测支付方式
        ...
        // 校验商品库存
        List<MyRecord> skuRecordList = validateProductStock(infoVo, user);
...
//            StoreProductAttrValue seckillAttrValue = storeProductAttrValueService.getByIdAndProductIdAndType(detailVo.getAttrValueId(), orderInfoVo.getSeckillId(), ProductType.PRODUCT_TYPE_SECKILL);
...
//
        ...
        orderComputedPriceRequest.setPreOrderNo(request.getPreOrderNo());
...
//        int gainIntegral = 0;
        ...
        for (OrderInfoDetailVo detailVo : infoVo.getOrderDetailList()) {
            //TODO 分离代码
//            // 赠送积分
...
//        }
        ...
        storeOrder.setSeckillId(infoVo.getSeckillId());
        storeOrder.setBargainId(infoVo.getBargainId());
        storeOrder.setBargainUserId(infoVo.getBargainUserId());
        storeOrder.setCreateTime(DateUtil.nowDateTime());
        storeOrder.setShippingType(request.getShippingType());
        storeOrder.setFreightPrice(infoVo.getFreightFee());
        storeOrder.setPaymentChannel(isChannel);
        storeOrder.setPaid(false);
        storeOrder.setCost(BigDecimal.ZERO);
        storeOrder.setType(0);
        storeOrder.setRefundPrice(new BigDecimal(0));
        storeOrder.setBeforePayPrice(infoVo.getPayFee());
        storeOrder.setDeductionPostage(infoVo.getDeductionPostage());
        if (infoVo.getIsVideo()) {
...
//                // 秒杀商品规格扣库存
//                storeProductAttrValueService.operationStock(skuRecord.getInt("activityAttrValueId"), skuRecord.getInt("num"), "sub", ProductType.PRODUCT_TYPE_SECKILL);
//                // 普通商品口库存
//                storeProductService.operationStock(skuRecord.getInt("productId"), skuRecord.getInt("num"), "sub");
//                // 普通商品规格扣库存
//                storeProductAttrValueService.operationStock(skuRecord.getInt("attrValueId"), skuRecord.getInt("num"), "sub", ProductType.PRODUCT_TYPE_NORMAL);
...
//                // 砍价商品规格扣库存
//                storeProductAttrValueService.operationStock(skuRecord.getInt("activityAttrValueId"), skuRecord.getInt("num"), "sub", ProductType.PRODUCT_TYPE_BARGAIN);
//                // 普通商品口库存
//                storeProductService.operationStock(skuRecord.getInt("productId"), skuRecord.getInt("num"), "sub");
//                // 普通商品规格扣库存
//                storeProductAttrValueService.operationStock(skuRecord.getInt("attrValueId"), skuRecord.getInt("num"), "sub", ProductType.PRODUCT_TYPE_NORMAL);
...
//                // 拼团商品规格扣库存
//                storeProductAttrValueService.operationStock(skuRecord.getInt("activityAttrValueId"), skuRecord.getInt("num"), "sub", ProductType.PRODUCT_TYPE_PINGTUAN);
//                // 普通商品口库存
//                storeProductService.operationStock(skuRecord.getInt("productId"), skuRecord.getInt("num"), "sub");
//                // 普通商品规格扣库存
//                storeProductAttrValueService.operationStock(skuRecord.getInt("attrValueId"), skuRecord.getInt("num"), "sub", ProductType.PRODUCT_TYPE_NORMAL);
...
//                }
            ...
            if (CollUtil.isNotEmpty(infoVo.getCartIdList())) {
                storeCartService.deleteCartByIds(infoVo.getCartIdList());
            }
        ...
        // 删除缓存订单
        if (redisUtil.exists("user_order:" + request.getPreOrderNo())) {
            redisUtil.delete("user_order:" + request.getPreOrderNo());
        }
    ...
    private List<MyRecord> validateProductStock(OrderInfoVo orderInfoVo, User user) {
        ...
        if (orderInfoVo.getSeckillId() > 0) {
            ...
            StoreProductAttrValue seckillAttrValue = storeProductAttrValueService.getByIdAndProductIdAndTypeNotDel(detailVo.getAttrValueId(), seckillId, ProductType.PRODUCT_TYPE_SECKILL);
            if (ObjectUtil.isNull(seckillAttrValue)) {
                throw new CrmebException("秒杀商品规格不存在");
            }
        ...
        if (orderInfoVo.getBargainId() > 0) {
            ...
            StoreProductAttrValue bargainAttrValue = storeProductAttrValueService.getByIdAndProductIdAndTypeNotDel(detailVo.getAttrValueId(), bargainId, ProductType.PRODUCT_TYPE_BARGAIN);
        ...
        if (orderInfoVo.getCombinationId() > 0) {
            ...
            StoreProductAttrValue combinationAttrValue = storeProductAttrValueService.getByIdAndProductIdAndTypeNotDel(detailVo.getAttrValueId(), combinationId, ProductType.PRODUCT_TYPE_PINGTUAN);
        ...
        orderDetailList.forEach(e -> {
            ...
            // 查询商品规格属性值信息
            StoreProductAttrValue attrValue = attrValueService.getByIdAndProductIdAndTypeNotDel(e.getAttrValueId(), e.getProductId(), ProductType.PRODUCT_TYPE_NORMAL);
        ...
        if ("buyNow".equals(request.getPreOrderType())) {// 立即购买
            ...
            if (detailRequest.getSeckillId() > 0) {// 秒杀
                ...
                if (request.getIsGiftCard() == 0)
                    attrValue = attrValueService.getByIdAndProductIdAndTypeNotDel(detailRequest.getAttrValueId(), detailRequest.getProductId(), ProductType.PRODUCT_TYPE_NORMAL);
                else
                    attrValue = attrValueService.getByIdAndProductIdAndType(detailRequest.getAttrValueId(), detailRequest.getProductId(), ProductType.PRODUCT_TYPE_NORMAL);
                if (ObjectUtil.isNull(attrValue)) {
                ...
                detailVo.setProductType(ProductType.PRODUCT_TYPE_NORMAL);
                detailVo.setVipPrice(detailVo.getPrice());
                detailVo.setGiveIntegral(storeProduct.getGiveIntegral());
                detailVo.setIsDeliver(storeProduct.getIsDeliver());
                detailVo.setIsPickup(storeProduct.getIsPickup());
    ...
    private List<OrderInfoDetailVo> validatePreOrderShopping(PreOrderRequest request, User user) {
        ...
        request.getOrderDetails().forEach(e -> {
            ...
            // 查询商品规格属性值信息
            if (request.getIsGiftCard() == 0)
                attrValue = attrValueService.getByIdAndProductIdAndTypeNotDel(Integer.valueOf(storeCart.getProductAttrUnique()), storeCart.getProductId(), ProductType.PRODUCT_TYPE_NORMAL);
            else
                attrValue = attrValueService.getByIdAndProductIdAndType(Integer.valueOf(storeCart.getProductAttrUnique()), storeCart.getProductId(), ProductType.PRODUCT_TYPE_NORMAL);
            if (ObjectUtil.isNull(attrValue)) {
                throw new CrmebException("商品规格信息不存在，请刷新后重新选择");
            }
            ...
            detailVo.setIsPickup(e.getIsPickup());
            detailVo.setIsDeliver(e.getIsDeliver());
            detailVoList.add(detailVo);
        });
        return detailVoList;
    }
    ...
    private OrderInfoDetailVo validatePreOrderSeckill(PreOrderDetailRequest detailRequest, User user) {
        ...
        StoreProductAttrValue seckillAttrValue = storeProductAttrValueService.getByIdAndProductIdAndTypeNotDel(detailRequest.getAttrValueId(), seckillId, ProductType.PRODUCT_TYPE_SECKILL);
        if (ObjectUtil.isNull(seckillAttrValue)) {
            throw new CrmebException("秒杀商品规格不存在");
        }
        ...
        detailVo.setProductType(ProductType.PRODUCT_TYPE_SECKILL);
        return detailVo;
    }
    ...
    private MyRecord commonValidateSeckill(StoreSeckill storeSeckill, StoreProductAttrValue seckillAttrValue, User user, Integer productNum) {
        ...
        // 主商品sku
        StoreProductAttrValue productAttrValue = storeProductAttrValueService.getByProductIdAndSkuAndType(storeSeckill.getProductId(), seckillAttrValue.getSuk(), ProductType.PRODUCT_TYPE_NORMAL);
        ...
        String stopTimeStr = DateUtil.dateToStr(storeSeckill.getStopTime(), DateConstants.DATE_FORMAT_DATE);
        Date stopDate = DateUtil.strToDate(stopTimeStr + " " + seckillManger.getEndTime() + ":00:00", DateConstants.DATE_FORMAT);
    ...
    private OrderInfoDetailVo validatePreOrderBargain(PreOrderDetailRequest detailRequest, User user) {
        ...
        StoreBargain storeBargain = storeBargainService.getByIdException(bargainId);
        StoreProductAttrValue bargainAttrValue = storeProductAttrValueService.getByIdAndProductIdAndTypeNotDel(detailRequest.getAttrValueId(), bargainId, ProductType.PRODUCT_TYPE_BARGAIN);
        if (ObjectUtil.isNull(bargainAttrValue)) {
            throw new CrmebException("砍价商品规格不存在");
        }
        ...
        detailVo.setProductType(ProductType.PRODUCT_TYPE_BARGAIN);
        return detailVo;
    }
    ...
    private MyRecord commonValidateBargain(StoreBargain storeBargain, StoreProductAttrValue bargainAttrValue, Integer bargainUserId, User user, Integer productNum) {
        ...
        // 主商品sku
        StoreProductAttrValue productAttrValue = storeProductAttrValueService.getByProductIdAndSkuAndType(storeBargain.getProductId(), bargainAttrValue.getSuk(), ProductType.PRODUCT_TYPE_NORMAL);
        if (ObjectUtil.isNull(productAttrValue)) {
    ...
    private OrderInfoDetailVo validatePreOrderCombination(PreOrderDetailRequest detailRequest, User user) {
        ...
        StoreProductAttrValue combinationAttrValue = storeProductAttrValueService.getByIdAndProductIdAndTypeNotDel(detailRequest.getAttrValueId(), combinationId, ProductType.PRODUCT_TYPE_PINGTUAN);
        if (ObjectUtil.isNull(combinationAttrValue)) {
            throw new CrmebException("拼团商品规格不存在");
        }
        ...
        detailVo.setProductType(ProductType.PRODUCT_TYPE_PINGTUAN);
        return detailVo;
    }
    ...
    private MyRecord commonValidateCombination(StoreCombination storeCombination, StoreProductAttrValue combinationAttrValue, User user, Integer productNum) {
        ...
        // 主商品sku
        StoreProductAttrValue productAttrValue = storeProductAttrValueService.getByProductIdAndSkuAndType(storeCombination.getProductId(), combinationAttrValue.getSuk(), ProductType.PRODUCT_TYPE_NORMAL);
        if (ObjectUtil.isNull(productAttrValue)) {
    ...
    private List<OrderInfoDetailVo> validatePreOrderAgain(PreOrderDetailRequest detailRequest, User user) {
        ...
        infoVoList.forEach(e -> {
            ...
            // 查询商品规格属性值信息
            StoreProductAttrValue attrValue = attrValueService.getByIdAndProductIdAndTypeNotDel(detailVo.getAttrValueId(), detailVo.getProductId(), ProductType.PRODUCT_TYPE_NORMAL);
            if (ObjectUtil.isNull(attrValue)) {
                throw new CrmebException("商品规格信息不存在，请刷新后重新选择");
            }
            ...
            tempDetailVo.setProductType(ProductType.PRODUCT_TYPE_NORMAL);
    ...
    private void getFreightFee(OrderInfoVo orderInfoVo, UserAddress userAddress) {
        ...
        AtomicReference<BigDecimal> storePostage = new AtomicReference<>(BigDecimal.ZERO);
        if ("true".equals(postageSwitchString) && ("0".equals(storeFreePostageString) || orderInfoVo.getProTotalFee().compareTo(new BigDecimal(storeFreePostageString)) >= 0)) {
            storePostage.set(BigDecimal.ZERO);
            ...
            // 根据 MyRecord 的 tempId 进行分组
            Map<Integer, List<Map<Integer, MyRecord>>> groupedRecords = proMap.entrySet().stream()
                    .collect(Collectors.groupingBy(
                            entry -> entry.getValue().get("tempId"),
                            Collectors.mapping(
                                    entry -> Collections.singletonMap(entry.getKey(), entry.getValue()),
                                    Collectors.toList()
                            )
                    ));
            for (Map.Entry<Integer, List<Map<Integer, MyRecord>>> entry : groupedRecords.entrySet()) {
                Integer tempId = entry.getKey();
                List<Map<Integer, MyRecord>> valueList = entry.getValue();
                List<MyRecord> myRecordList = valueList.stream()
                        .flatMap(map -> map.values().stream())
                        .collect(Collectors.toList());

                BigDecimal totalPrice = myRecordList.stream().map(r -> r.getBigDecimal("totalPrice")).reduce(BigDecimal.ZERO, BigDecimal::add);
                int totalNum = myRecordList.stream().mapToInt(r -> r.getInt("totalNum")).sum();
                BigDecimal volume = myRecordList.stream().map(r -> r.getBigDecimal("volume")).reduce(BigDecimal.ZERO, BigDecimal::add);
                BigDecimal weight = myRecordList.stream().map(r -> r.getBigDecimal("weight")).reduce(BigDecimal.ZERO, BigDecimal::add);
                ...
                switch (shippingTemplate.getType()) {
                    case 1: // 件数
                        // 判断件数是否超过首件
                        if (totalNum <= shippingTemplatesRegion.getFirst().intValue()) {
                            storePostage.set(storePostage.get().add(shippingTemplatesRegion.getFirstPrice()));
                        } else {// 超过首件的需要计算续件
                            int renewalNum = totalNum - shippingTemplatesRegion.getFirst().intValue();
                            ...
                            storePostage.set(storePostage.get().add(shippingTemplatesRegion.getFirstPrice()).add(renewalPrice));
                        }
                        break;
                    case 2: // 重量
                ...
                //计算减免邮费
                //满额减邮/包邮
                if (Objects.nonNull(shippingTemplate.getFullReductionAmount())) {
                    if (shippingTemplate.getFullReductionAmount().compareTo(orderInfoVo.getProTotalFee()) <= 0) {
                        if (shippingTemplate.getIsFreePostage()) {
                            //包邮开关开启 并且支付前商品总额大于满减金额 包邮
                            orderInfoVo.setDeductionPostage(storePostage.get());
                            storePostage.set(BigDecimal.ZERO);
                        } else {
                            //满减邮费
                            if (storePostage.get().compareTo(BigDecimal.ZERO) > 0) {
                                BigDecimal subtract = storePostage.get().subtract(shippingTemplate.getDeductionPostage());
                                if (subtract.compareTo(BigDecimal.ZERO) <= 0) {
                                    storePostage.set(BigDecimal.ZERO);
                                    orderInfoVo.setDeductionPostage(storePostage.get());
                                } else {
                                    orderInfoVo.setDeductionPostage(shippingTemplate.getDeductionPostage());
                                    storePostage.set(subtract);
                                }
                            }
                        }
                    }
        ...
        orderInfoVo.setFreightFee(storePostage.get());
    }
